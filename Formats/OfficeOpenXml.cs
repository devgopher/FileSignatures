    ...
    public abstract class OfficeOpenXml : Zip, IFileFormatReader
    {
        ...
        public bool IsMatch(IDisposable? file)
        {
            if (file is ZipArchive archive)
            {
                ...
                return archive.Entries.Any(e => e.FullName.StartsWith(fileName, StringComparison.OrdinalIgnoreCase) && e.FullName.EndsWith(extension, StringComparison.OrdinalIgnoreCase));
            }
